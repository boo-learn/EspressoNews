...
def test_create_tg_account(client: TestClient, superuser_token_headers):
    ...
    result = client.post(f"{GPTACCOUNTS_URL}", json=account_data, headers=superuser_token_headers)
    account = result.json()
    assert result.status_code == 200
    schemas.GPTAccountSchema(**account)
...
def test_tg_account_update(session, create_object, client: TestClient, superuser_token_headers):
    ...
    create_object(GPTAccount, account_data)
    result = client.put(f"{GPTACCOUNTS_URL}/1", json=edited_account_data, headers=superuser_token_headers)
...
def test_tg_account_delete(session, create_object, client: TestClient, superuser_token_headers):
    ...
    create_object(GPTAccount, account_data)
    result = client.delete(f"{GPTACCOUNTS_URL}/1", headers=superuser_token_headers)
