...
@pytest.mark.asyncio
async def test_simple_send_receive_message(event_loop, rabbitmq_server):
    @add_args
    def on_message(data):
        print(f"{data=}")
    rabbit_port = rabbitmq_server["rabbit_port"]
    host = f"localhost:{rabbit_port}"
    producer = Producer(host=host, queue=QueuesType.ping)
    message: MessageData = {
        "type": "test_message",
        "data": "test_data"
    }
    await producer.send_message(message_with_data=message, queue=QueuesType.ping)
    subscriber = Subscriber(host=host, queue=QueuesType.ping)
...
@pytest.mark.skip
@pytest.mark.asyncio
async def test_messages_prepare_digest(event_loop, session, load_data_from_json, rabbitmq_server):
    @add_args
